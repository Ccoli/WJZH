...
namespace ModBus
{
    ...
    public class CommonHelper
    {
        ...
        {
            return DateTime.Now.ToString("yyyy-MM-ddTHH:mm:sszzzz", DateTimeFormatInfo.InvariantInfo);
        }
        public static DateTime ToDateTimeFromUTCISO8601(string dateTimeString)
        {
            DateTime dt;
            var sucessed = DateTime.TryParseExact(dateTimeString, new string[] { @"yyyy-MM-ddTHH:mm:ss.ssssssss\Z", "o" }, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out dt);

            return dt;
        }

        public static int[] GetUTF8(string code)
        {
            byte[] bytes = System.Text.Encoding.UTF8.GetBytes(code); //string转换的字母
        ...
        public static string GetUTF8Str(int[] receive)
        {

            byte[] i = new byte[receive.Length];
            for (int x = 0; x < receive.Length; x++)
            {
                i[x] = (byte)receive[x];
            }
            string str = System.Text.Encoding.UTF8.GetString(i);
            return str;
        }
        /// <summary>
        /// 数组转换成逗号分隔的字符串
        ...
        /// <returns></returns>
        public static string GetArrayStr(int[] list)
        {
        ...
        /// <returns></returns>
        public static int Hex2Ten(string code)
        {
            return Int32.Parse(code, System.Globalization.NumberStyles.HexNumber);
        }
        /// <summary>
        /// 从十进制转换到十六进制
        /// </summary>
